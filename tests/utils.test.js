...
const { 
    urlExtractor, checkInputArgs, getDomainName, getPageData, crawl, isUrl
} = require('../src/utils');
...
test('Check if url returns false', () => {
    expect(isUrl('github.com')).toBe(false);
});
...
test('Check input args, invalid command returns undefined', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Crawl: Invalid command. See crawl --help');
...
test('Check input args, logs help information and returns undefined', () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Usage: crawl [[arguments] url] [options]\n\nOptions:\n   --help          Show help\n\nargument:\n   -n <Integer>    Set Number of workers (Optional)\n\nSamples:\n./Crawl https://www.amazon.com\n./Crawl -n 6 https://www.amazon.com');
    expect(result).toBe(undefined);
});
...
test('Should return valid urls given valid inputs', () => {
    ...
    const response = crawl(url, domainName, data, seenUrls);
