...
const { getDomainName, crawl, checkInputArgs, getPageData } = require('./utils');
...
(async () => {
    if (isMainThread) {
        ...
        if(numWorkers == 0) {
            (async () => {
                while (urlsQueue.size() > 0) {
                    ...
                    const validUrls = crawl(curUrl, domainName, pageData, seenUrls);
        ...
        parentPort.on('message', async (data) => {
            ...
            parentPort.postMessage({validUrls: crawl( data.curUrl, data.domainName, pageData, data.seenUrls), id: data.id });
        })
    }
})();